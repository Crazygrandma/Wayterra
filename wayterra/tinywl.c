...
#define MOVE_REPEAT_DELAY_MS 10
#define MOVE_REPEAT_RATE_HZ 60
#define MOVE_REPEAT_STEP 40

static void move_all_toplevels(struct tinywl_server *server, int dx, int dy) {
...
static bool move_vector_for_sym(xkb_keysym_t sym, int *dx, int *dy) {
	switch (sym) {
	case XKB_KEY_a:
	case XKB_KEY_A:
		*dx = MOVE_REPEAT_STEP;
...
static void start_move_repeat(struct tinywl_server *server, int dx, int dy) {
	server->move_repeat_dx = dx;
	server->move_repeat_dy = dy;
	server->move_repeat_active = true;
	wl_event_source_timer_update(server->move_repeat_timer, MOVE_REPEAT_DELAY_MS);
}

static void stop_move_repeat(struct tinywl_server *server) {
	server->move_repeat_active = false;
	wl_event_source_timer_update(server->move_repeat_timer, 0);
}

static int move_repeat_timer_handler(void *data) {
	struct tinywl_server *server = data;
	if (!server->move_repeat_active) {
		return 0;
	}

	move_all_toplevels(server, server->move_repeat_dx, server->move_repeat_dy);
	wl_event_source_timer_update(server->move_repeat_timer,
		1000 / MOVE_REPEAT_RATE_HZ);
	return 0;
}
...
static bool handle_keybinding(struct tinywl_server *server, xkb_keysym_t sym) {
	 ...
	 * processing.
	 *
	 * This function is called after keyboard_handle_key() applies modifier rules.
	 */
	switch (sym) {
	case XKB_KEY_Escape:
	...
	case XKB_KEY_Tab:
		...
		if (!server->movement_mode_enabled) {
			stop_move_repeat(server);
		}
		wlr_log(WLR_INFO, "movement mode %s",
			server->movement_mode_enabled ? "enabled" : "disabled");
		break;
	case XKB_KEY_a:
	case XKB_KEY_A:
		move_all_toplevels(server, MOVE_REPEAT_STEP, 0);
		break;
	case XKB_KEY_d:
	case XKB_KEY_D:
		move_all_toplevels(server, -MOVE_REPEAT_STEP, 0);
	...
	case XKB_KEY_w:
	case XKB_KEY_W:
		move_all_toplevels(server, 0, MOVE_REPEAT_STEP);
		break;
	case XKB_KEY_s:
	case XKB_KEY_S:
		move_all_toplevels(server, 0, -MOVE_REPEAT_STEP);
...
static void keyboard_handle_key(
	...
	for (int i = 0; i < nsyms; i++) {
		...
		bool is_move = move_vector_for_sym(syms[i], &move_dx, &move_dy);

		if (event->state == WL_KEYBOARD_KEY_STATE_PRESSED &&
				is_move && server->movement_mode_enabled) {
			if (handle_keybinding(server, syms[i])) {
				handled = true;
			}
			start_move_repeat(server, move_dx, move_dy);
			continue;
		}
...
static void server_new_keyboard(struct tinywl_server *server,
	...
	rules.layout = DEFAULT_XKB_LAYOUT;
...
static void output_frame(struct wl_listener *listener, void *data) {
	...
	wlr_scene_node_set_position(&output->blue_background->node,
...
static void xdg_toplevel_map(struct wl_listener *listener, void *data) {
	...
	// Intial placement of wayland clients
...
int main(int argc, char *argv[]) {
	...
	server.move_repeat_timer = wl_event_loop_add_timer(event_loop,
		move_repeat_timer_handler, &server);
	if (server.move_repeat_timer == NULL) {
		wlr_log(WLR_ERROR, "failed to create move repeat timer");
		return 1;
	}
